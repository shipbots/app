...
function buildPrompt(rows: Record<string, unknown>[]): string {
  ...
  1. Missing or invalid postal codes. Every row that has a Country Code needs a Zip. US zips are 5 digits (or 5+4). CA zips look like "A1A 1A1". GB is alphanumeric. If a Zip is blank OR clearly wrong shape for the country, flag it. If the row also carries an address that includes an obvious zip you can extract, provide it as "suggestion". EXCEPTION: the United Arab Emirates ("AE", includes Dubai) has NO postal code system. For AE rows, the formatter auto-fills Zip with "00000" — if you see "00000" on an AE row, that is CORRECT and must NOT be flagged. If you see a blank Zip on an AE row, suggest "00000".
